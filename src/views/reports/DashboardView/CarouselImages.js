...
const CarouselImages = ({ className, ...rest }) => {
  ...
  const handleButtonClick = () => {
    let ele = document.getElementById('button-file');
    ele.click();
  };

  const getCarouselImages = () => {
    const dbRef = firebase.database().ref('carousel');
  ...
  const uploadFile = async (file, name) => {
    let reference = firebase.storage().ref('carouselImages');
  ...
  const saveRef = (url, name) => {
    const ref = firebase.database().ref('carousel');
    let obj = {
      URL: url,
      ImageName: name
    };
      ...
      .then(() => {
        dataRef
          ...
          .then(() => {
            store.addNotification({
              title: "'Successful!",
              message: 'Image Deleted from Carousel',
              type: 'success',
              insert: 'top',
              container: 'bottom-right',
              animationIn: ['animate__animated', 'animate__fadeIn'],
