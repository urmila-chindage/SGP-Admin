...
const Library = () => {
  ...
  const getData = () => {
    const dbRef = firebase.database().ref('libraryData');
    dbRef.child('REPORT').on('value', datasnapshot => {
      if (datasnapshot.val()) {
        // datasnapshot.val()["Computer Science"].forEach(val => {
        //   console.log(val)
        // })
    ...
    dbRef.child('ACHIEVEMENT').on('value', datasnapshot => {
      if (datasnapshot.val()) {
        // datasnapshot.val()["Computer Science"].forEach(val => {
        //   console.log(val)
        // })
    ...
    dbRef.child('BOOKBANK').on('value', datasnapshot => {
      if (datasnapshot.val()) {
        // datasnapshot.val()["Computer Science"].forEach(val => {
        //   console.log(val)
        // })
    ...
    <Page className={classes.root} title="Academic Calendar">
      <Container maxWidth={false}>
        ...
        <Box mt={3}>
          ...
          <Results data={achievements} />
          <Results data={bookbank} />
        </Box>
        <Drawer
